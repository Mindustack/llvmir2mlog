...
import org.mindustack.llvmir2mlog.compiler.backend.rvasm.operand.*;
import org.mindustack.llvmir2mlog.compiler.backend.rvasm.operand.RawStackOffset.RawType;
...
import org.mindustack.llvmir2mlog.compiler.share.pass.IRModulePass;
...
public class AsmBuilder implements IRModulePass, IRFuncPass, IRBlockPass, InstVisitor {
    ...
    private static boolean equalZero(Value value) {
        return value instanceof NullptrConst || (value instanceof NumConst && ((NumConst) value).getConstData() == 0)
                || (value instanceof BoolConst && !((BoolConst) value).constData);
    }
    ...
    public void buildModuleSkeleton(IRModule irModule) {
        ...
        module.mainFunction = module.functions.stream()
                .filter(asmFunction -> asmFunction.identifier.equals(MLOG.MainFunctionIdentifier)).findFirst().get();
    ...
    public void runOnFunc(IRFunction function) {
        ...
        new AsmALUInst(MLOG.AddOperation, PhysicalReg.reg("sp"), PhysicalReg.reg("sp"),
                new RawStackOffset(0, RawStackOffset.RawType.lowerSp), cur.func.entryBlock);
        ...
        new AsmALUInst(MLOG.SubOperation, PhysicalReg.reg("sp"), PhysicalReg.reg("fp"),
                new RawStackOffset(0, RawStackOffset.RawType.stackUse), cur.func.entryBlock);
...
cur.func.blocks.forEach(block -> {
            block.instructions.forEach(inst -> {
                if (!(inst.imm instanceof RawMemOffset))
                    return;
    ...
    // public Register awesomeGEP(Value ptrPos, Value index, int elementSize,
    ...
    // new AsmALUInst(MLOG.AddOperation, gepReg, cur.toReg(ptrPos), mulReg,
    // cur.block);
    // }
    // }
    // return gepReg;
    ...
    public void visit(IRAllocaInst inst) {
        ...
        if (cur.block == cur.func.entryBlock) {
            ...
            new AsmALUInst(MLOG.SubOperation, PhysicalReg.reg("sp"), PhysicalReg.reg("sp"),
                    cur.toImm(((PointerType) inst.type).pointedType.size()), cur.block);
            new AsmMoveInst(cur.toReg(inst), PhysicalReg.reg("sp"), cur.block);

            // awesomeALU(MLOG.SubOperation,
            // cur.toReg(inst),PhysicalReg.reg("sp"),cur.toImm(((PointerType)
            // inst.type).pointedType.size()) );
    ...
    public void visit(IRBrInst inst) {
        ...
        if (inst.condition() instanceof IRICmpInst && ((IRICmpInst) inst.condition()).forBr()) {
            ...
            if (result.second())
                new AsmBrInst(result.first(), cur.toReg(((IRICmpInst) inst.condition()).rhs()),
                        cur.toReg(((IRICmpInst) inst.condition()).lhs()), (AsmBlock) inst.ifTrueBlock().asmOperand,
                        cur.block);
            else
                new AsmBrInst(result.first(), cur.toReg(((IRICmpInst) inst.condition()).lhs()),
                        cur.toReg(((IRICmpInst) inst.condition()).rhs()), (AsmBlock) inst.ifTrueBlock().asmOperand,
                        cur.block);
        } else {
    ...
    public void visit(IRCallInst inst) {
        ...
        // for (PhysicalReg phyReg : PhysicalReg.calleeSaved) {
        // // break;
        // new AsmALUInst(MLOG.SubOperation, PhysicalReg.reg("sp"),
        // PhysicalReg.reg("sp"), new Immediate(1), cur.block);
        //
        // //VirtualReg rd = new VirtualReg();
        // //calleeSaveTemp.add(rd);
        //
        ...
        // new Immediate(0),//new
        // RawStackOffset(callFunc.arguments.get(i).stackOffset.value,
        // RawType.callerArg),
        // cur.block);
        ...
        for (int i = 0; i < Integer.min(inst.callFunc().getArgNum(), MLOG.MaxArgRegNum); i++) {
            if (inst.getArg(i) instanceof GlobalValue) {
                if (((GlobalValue) inst.getArg(i)).gpRegMark != 0) {
                    // todo
                    new AsmMoveInst(PhysicalReg.a(i), PhysicalReg.gp(((GlobalValue) inst.getArg(i)).gpRegMark),
                            cur.block);
                } else {
                    // new AsmLaInst(PhysicalReg.a(i), inst.getArg(i).asmOperand.identifier,
                    // cur.block);
                }
            } else
                awesomeMove(PhysicalReg.a(i), inst.getArg(i));
        }
        Register argInStack;
        // spill to mem
        for (int i = MLOG.MaxArgRegNum; i < callFunc.arguments.size(); i++) {
            ...
            new AsmStoreInst(argInStack,
                    // notice: here use the argument of CallInst, not the func
                    // and the RawStackOffset should use caller
                    cur.toReg(inst.getArg(i)), new Immediate(0), // new
                                                                 // RawStackOffset(callFunc.arguments.get(i).stackOffset.value,
                                                                 // RawType.callerArg),
                    cur.block);
        }
        ...
        // cur.func.callerArgStackUse = max(cur.func.callerArgStackUse,
        // callFunc.calleeArgStackUse);
        ...
        // // callee temp back
        // for (PhysicalReg phyReg : PhysicalReg.calleeSaved) {
        // new AsmALUInst(MLOG.AddOperation, PhysicalReg.reg("sp"),
        // PhysicalReg.reg("sp"), new Immediate(1), cur.block);
        //// break;
        // //VirtualReg rd = new VirtualReg();
        // //calleeSaveTemp.add(rd);
    ...
    public void visit(IRGetElementPtrInst inst) {
        ...
        // StructType classType = inst.isGetMember() ? (StructType) ((PointerType)
        // inst.headPointer().type).pointedType : null;
        // int elementSize = ((PointerType) inst.headPointer().type).pointedType.size();
        // // well... quite interesting
        //
        // Register instReg = cur.toReg(inst);
        // Register gepReg = awesomeGEP(inst.headPointer(), index, elementSize,
        // classType);
        ...
        for (int i = 2; i < inst.operandSize(); ++i) {
            ...
            // awesomeALU(MLOG.MulOperation, virtualReg, operand, new
            // NumConst(elementSize));
            //
            //
            //
            ...
            if (curElement instanceof ArrayType) {
                ...
                //
                // new AsmLiInst(virtualReg, cur.toImm(((NumConst) operand).constData),
                // cur.block);
        ...
        //
        // if (index instanceof NumConst && !(inst.headPointer() instanceof GlobalValue)
        // && specialGEPCheck(inst)) {
        // int offset = 0;
        // if (classType != null) {
    ...
    public void visit(IRLoadInst inst) {
        ...
        if (inst.loadPtr() instanceof GlobalValue) {
            if (((GlobalValue) inst.loadPtr()).gpRegMark != 0) {
                ...
                new AsmLoadInst(inst.type.size(), instReg, PhysicalReg.reg("zero"), cur.toImm(inst.loadPtr()),
                        cur.block);
            ...
            //// new AsmLuiInst(luiReg, new GlobalAddr(globalReg, GlobalAddr.HiLo.hi),
            //// cur.block);
            ...
            // new AsmLoadInst(inst.type.size(), instReg, cur.toReg(inst.loadPtr()),
            // cur.toImm(0), cur.block);
    ...
    public void visit(IRStoreInst inst) {
        if (inst.storePtr() instanceof GlobalValue) {
            if (((GlobalValue) inst.storePtr()).gpRegMark != 0) {
                new AsmMoveInst(PhysicalReg.gp(((GlobalValue) inst.storePtr()).gpRegMark), cur.toReg(inst.storeValue()),
                        cur.block);
            } else {
                // todo VirtualReg luiReg = new VirtualReg();
                ...
                // new AsmLuiInst(luiReg, new GlobalAddr(globalReg, GlobalAddr.HiLo.hi),
                // cur.block);
                new AsmStoreInst(PhysicalReg.reg("zero"), cur.toReg(inst.storeValue()), cur.toImm(inst.storePtr()),
                        cur.block);
            }
        } else {
            ...
            if (inst.storePtr().asmOperand instanceof RawStackOffset) {
                // must be stack
                new AsmStoreInst(PhysicalReg.reg("fp"), cur.toReg(inst.storeValue()), cur.toImm(inst.storePtr()),
                        cur.block);
    ...
    // if (!(user instanceof IRLoadInst || user instanceof IRStoreInst)) return
    // false;
    // return true;
    // }
