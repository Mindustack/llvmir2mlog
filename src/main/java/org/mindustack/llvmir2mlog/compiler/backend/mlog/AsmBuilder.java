...
public class AsmBuilder implements IRModulePass, IRFuncPass, IRBlockPass, InstVisitor {
    ...
    public void runOnFunc(IRFunction function) {
        ...
        new AsmALUInst(MLOG.AddOperation, PhysicalReg.reg("sp"), PhysicalReg.reg("sp"), new RawStackOffset(0, RawStackOffset.RawType.lowerSp), cur.func.entryBlock);
        ...
        for (int i = MLOG.MaxArgRegNum; i < cur.func.arguments.size(); i++) {
            ...
            new AsmALUInst(MLOG.SubOperation, argInStack, PhysicalReg.reg("sp"), cur.func.arguments.get(i).stackOffset, cur.func.entryBlock);
...
//        new AsmLoadInst(1, PhysicalReg.reg("fp"), PhysicalReg.reg("fp"), new Immediate(0), cur.func.exitBlock);
        new AsmALUInst(MLOG.AddOperation, PhysicalReg.reg("sp"),
                PhysicalReg.reg("sp"),
                new RawStackOffset(0, RawStackOffset.RawType.raiseSp), cur.func.entryBlock);
    ...
    public void visit(IRStoreInst inst) {
        if (inst.storePtr() instanceof GlobalValue) {
            ...
            if (inst.storePtr().asmOperand instanceof RawStackOffset) {
                // must be stack
                new AsmStoreInst(PhysicalReg.reg("sp"), cur.toReg(inst.storeValue()), cur.toImm(inst.storePtr()), cur.block);
